...
(
	cat "/etc/telegraf/telegraf.conf.tpl" | sed "s|{{TELEGRAF_AGENT_COLLECT_INTERVAL}}|${TELEGRAF_AGENT_COLLECT_INTERVAL}|g;s|{{TELEGRAF_AGENT_FLUSH_INTERVAL}}|${TELEGRAF_AGENT_FLUSH_INTERVAL}|g"
	echo "${TELEGRAF_OUTPUTS}"
) > "/tmp/telegraf.conf"
